...
export class basePage {
  ...
  public async fillText(element: Locator, textToFill: any) {
    await test.step(`Filling '${textToFill}' into the '${element}' element`, async () => {
      await element.fill(textToFill);
    });
  }

  public async fillTextSequentially(element: Locator, textToFill: any) {
    await test.step(`Filling '${textToFill}' into the '${element}' element`, async () => {
      await element.pressSequentially(textToFill);
    });
  }
  ...
  async printAllTheListElements(element: any) {
    await this.page.waitForSelector(element, { timeout: 30000 });
    const list = await this.page.$$(element);
    console.log(list.length);

    const allLists = await Promise.all(
      list.map(async (list, i) => {
        return await list.innerText();
      })
    );
    console.log(allLists);
  }

  async focusOnElement(element: Locator) {
    await element.focus();
  }
